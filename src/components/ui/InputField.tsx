  ...
  React.forwardRef<TextInput, InputFieldProps>(({
    ...
    return (
      <View className="mb-4">
        ...
        <TextInput
          ref={ref}
          className={`bg-slate-100 dark:bg-slate-700 rounded-lg p-4 text-base text-slate-900 dark:text-slate-100 border ${
            error
              ? 'border-expense'
              : 'border-transparent'
          } ${!editable ? 'opacity-60 bg-slate-200 dark:bg-slate-600' : ''}`}
