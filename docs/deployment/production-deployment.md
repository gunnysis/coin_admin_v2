...
프로덕션 배포 체크리스트, Android/iOS/OTA 절차, Sentry·EAS 환경변수(Secrets), 롤백 안내.
...
- **Sentry 소스맵 토큰(빌드 차단 요소):** production EAS 빌드는 `@sentry/react-native/expo` 플러그인이 소스맵을 업로드한다 — EAS secret **`SENTRY_AUTH_TOKEN`**(production 환경)이 없거나 만료되면 **업로드 단계가 빌드 자체를 실패시킨다**([app.config.ts](../../app.config.ts) 플러그인 주석 참고). `npx eas-cli env:list --environment production`으로 존재를 확인한다.
- **CI:** GitHub CI(ci.yml)와 EAS 워크플로는 같은 main push에 **각각 독립적으로** 실행되며 둘 사이에 순서 의존은 없다 — 배포를 실제로 차단하는 최종 게이트는 EAS 워크플로의 pre-build checks job(sync check·typecheck·jest)이다. GitHub CI는 조기 경보(추가로 웹 E2E·expo-doctor·npm audit 수행).
...
- **자동:** `main` 브랜치 push 시 [.eas/workflows/android-production.yml](../../.eas/workflows/android-production.yml)이 실행된다(checks → 빌드 → Play Store 제출. docs/·`*.md`(루트 포함)·`.github/`·`.eas/`만 변경된 push는 제외). Expo 대시보드에서 GitHub 저장소가 연결되어 있어야 한다.
...
- **JS/에셋만 변경:** `eas update --channel production -m "<메시지>"` — 채널이 매핑된 브랜치로 발행된다. **`--branch`와 `--channel`은 동시 지정 불가** — eas-cli가 "Cannot specify both --channel and --branch"로 거부한다(2026-07-29 CLI 소스 실측).
