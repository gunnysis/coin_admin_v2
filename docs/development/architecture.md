...
- **`src/lib/`**: 범용·인프라용 유틸. 에러 타입/정규화(`errors`), 로거(`logger`), React Query 유틸(`react-query`), 스토리지(`storage`), DB 헬퍼(`db-utils`), 범용 검증 프리미티브(`validation`), 배열/객체/성능 유틸(`array`, `object`, `performance`, `hooks`), 에러 리포팅(`errorReporting`), **백업**(`backup/`: 스냅샷 타입, 어댑터 인터페이스, `backupService`, `LocalBackupAdapter`) 등 앱에 종속되지 않는 유틸. hooks·서비스 레이어에서 사용.
...
주의: `lib/validation.ts`(범용 프리미티브: `validateRequired`·`validateRange` 등)와 `utils/validation.ts`(도메인 규칙: `validateExpenseForm` 등)가 공존하며 **둘 다 `ValidationResult` 인터페이스를 내보낸다** — import 시 출처를 혼동하지 말 것(도메인 검증은 항상 `@/utils/validation`).
