...
- **키보드:** 모달은 `KeyboardAvoidingView` 사용; Android는 `app.config.ts`에서 `softwareKeyboardLayoutMode: "pan"`. 상태바는 Android에서 `statusBar.translucent: false`로 콘텐츠가 상태바 아래로 그려지지 않도록 함.
