...
> - v1.5(2026-07-28): **v1.4 ①의 해석 정정 + DSN 근본 교정 완료.** 사용자 확인 결과 EAS에 등록됐던 DSN은 **gns-hermit-comm 프로젝트의 것**(coin-admin DSN을 몰라 실수 등록) — v1.4의 rename은 남의 프로젝트를 바꾼 것이라 **원상 복구**(name·slug `gns-hermit-comm`), "타 앱 이벤트 유입"은 오염이 아니라 그 프로젝트 자신의 이벤트였음(3-2-4 정정). 사용자가 Sentry wizard로 **전용 프로젝트 `gunnys/coin-admin`(ID 4511812698767360) 신규 생성** → EAS production `EXPO_PUBLIC_SENTRY_DSN`을 새 DSN으로 교체·검증 이벤트 HTTP 200 수신, 기본 알림 룰 자동 생성 확인. wizard가 main 워킹트리에 남긴 부작용(로컬 npm 11.6 lockfile 오염 — EUSAGE 패턴, metro 패치, .gitignore 중복)은 **전부 revert**(metro 전환은 준비 브랜치에 이미 존재). **주의: 빌드 47은 구 DSN이 빌드에 인라인돼 있어 롤아웃 기간 동안 gns-hermit-comm 프로젝트로 보고됨** — release `com.gunny.coinadmin.android@2.6.1+47` 필터로 모니터링, 새 DSN은 다음 빌드부터.
