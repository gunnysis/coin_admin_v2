...
2. **고정비 추가(원화)**: 고정비 탭 → 항목 추가 → 이름·금액(원)·결제일 입력 → 추가하기 → **모달 닫힘 확인**(현재 스펙은 모달 헤딩 사라짐까지만 검증 — 목록/총액 반영 어서션은 미구현, 보강 과제)
3. **유동비 추가(원화)**: 유동비 탭 → 항목 추가 → 이름·금액(원)·지출일 입력 → 추가하기 → **모달 닫힘 확인**(위와 동일 — 목록 반영 어서션 미구현)
